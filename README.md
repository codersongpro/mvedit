...
Phase 15까지 완료(계획한 단계 전부). 파일 불러오기, 타임라인 편집(자르기·트림·지우기·순서·되돌리기),
빈 화면·이미지 클립, 볼륨·음소거, 미리보기 재생, 자막 입력·굽기·`.srt` 내보내기,
화면비·해상도·화질·목표 용량 설정, 자동 저장·복원, `.cutcap` 프로젝트 파일,
모바일 세로 레이아웃, 홈 화면 설치·오프라인 실행, 입력 한계 경고까지 동작합니다.

남은 것은 실기기 성능 확인입니다 — 인코딩 시간·발열·저사양 폰에서의 성공 여부는
자동 검증으로 대신할 수 없어 실제 기기에서 재야 합니다.
...
그래야 `SharedArrayBuffer` 를 쓸 수 있습니다.

지금 내보내기는 WebCodecs로 하며 여기에 교차 출처 격리가 필요하지는 않습니다. 그래도
헤더를 유지하는 이유는, 나중에 ffmpeg.wasm 같은 여러 스레드 인코더를 붙일 때 이 공유
메모리가 필요하고(14단계에서 폴백을 넣지 않기로 한 근거는 PRD 14절에 적혀 있습니다),
배포 후에 헤더를 켜면 이미 캐시된 페이지와 상태가 어긋나기 때문입니다. 지금 끄는 것보다
켜 두는 쪽이 비용이 작습니다.
...
npm run test:perf      # 클립 100개 부하, 입력 한계 경고, 키보드 전용 경로
